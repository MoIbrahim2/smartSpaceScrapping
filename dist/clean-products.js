...
const relevanceClassifier_js_1 = require("./normalizers/relevanceClassifier.js");
const priceNormalizer_js_1 = require("./normalizers/priceNormalizer.js");
const dimensions_js_1 = require("./normalizers/dimensions.js");
const dimensionsSanitizer_js_1 = require("./normalizers/dimensionsSanitizer.js");
...
const qualityScorer_js_1 = require("./normalizers/qualityScorer.js");
const duplicateDetector_js_1 = require("./normalizers/duplicateDetector.js");
const catalogExpander_js_1 = require("./normalizers/catalogExpander.js");
...
const INPUT_PATH = path_1.default.join(process.cwd(), options.input);
const OUTPUT_DIR = path_1.default.join(process.cwd(), options.outputDir);
const REPORTS_DIR = path_1.default.join(process.cwd(), options.reportsDir);
...
async function runPipeline() {
    ...
    const rawData = JSON.parse(fs_1.default.readFileSync(INPUT_PATH, 'utf8'));
    ...
    for (const raw of rawData) {
        ...
        const classResult = (0, relevanceClassifier_js_1.classifyProduct)(rawName, rawCategory, rawDesc, rawSpecs);
        // 2. Price Normalization
        const priceResult = (0, priceNormalizer_js_1.normalizePrice)(raw.pricing?.currentPrice ?? raw.currentPrice, raw.pricing?.originalPrice ?? raw.originalPrice);
        // 3. Dimensions Sanitization
        const extractedDims = (0, dimensions_js_1.extractDimensions)(rawSpecs, rawName, rawDesc, classResult.canonicalCategory);
        const sanitizedDims = (0, dimensionsSanitizer_js_1.sanitizeDimensions)({
        ...
        }, rawName, rawDesc, classResult.canonicalCategory);
        // 4. Color, Material, Style mapping
        const combinedTextForAttrs = `${rawName} ${rawDesc} ${rawCategory} ${rawSub}`;
        const colors = (0, color_js_1.normalizeColors)(combinedTextForAttrs);
        const materials = (0, material_js_1.normalizeMaterials)(combinedTextForAttrs);
        const styles = (0, style_js_1.normalizeStyles)(combinedTextForAttrs, classResult.canonicalCategory);
        // 5. AI Rich Embedding Text
        const aiData = (0, aiEnrichment_js_1.enrichProductAI)(rawName, rawDesc, raw.basic?.brand || raw.brand || 'Generic', classResult.canonicalCategory, styles, colors, materials, classResult.roomTypes);
        const issues = [...sanitizedDims.issues];
        // Format formatted images list
        const formattedImages = (raw.images || []).map((img, idx) => ({
        ...
        const qualityResult = (0, qualityScorer_js_1.calculateQualityScore)(intermediateProduct, classResult.canonicalCategory);
    ...
    // Deduplication
    const { mergedProducts: dedupedProducts, duplicateCandidatesReport } = (0, duplicateDetector_js_1.detectAndProcessDuplicates)(acceptedList);
    let cleanProducts = dedupedProducts;
    if (options.expand) {
        cleanProducts = (0, catalogExpander_js_1.expandCatalog)(cleanProducts, 25);
    }
    ...
    const roomCoverageReport = {};
    const missingCategoriesList = [];
    const lowCoverageCategoriesList = [];
    ROOMS_LIST.forEach(roomInfo => {
        ...
        roomInfo.categories.forEach(cat => {
            ...
            if (cnt === 0) {
                if (!missingCategoriesList.includes(cat))
                    missingCategoriesList.push(cat);
            }
            else if (cnt < 5) {
    ...
    if (!options.dryRun) {
        fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'products_clean_3d.json'), JSON.stringify(cleanProducts, null, 2));
        fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'products_review.json'), JSON.stringify(reviewList, null, 2));
        fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'products_rejected.json'), JSON.stringify(rejectedList, null, 2));
        fs_1.default.writeFileSync(path_1.default.join(OUTPUT_DIR, 'duplicate_candidates.json'), JSON.stringify(duplicateCandidatesReport, null, 2));
    }
    ...
    fs_1.default.writeFileSync(path_1.default.join(REPORTS_DIR, 'category-mapping-report.json'), JSON.stringify(mappingReportJson, null, 2));
    ...
    fs_1.default.writeFileSync(path_1.default.join(REPORTS_DIR, 'cleaning-report.json'), JSON.stringify(cleaningReportJson, null, 2));
...
function compileIssuesSummary(cleanList, reviewList) {
    const issuesMap = {};
    cleanList.forEach(p => {
...
function generateCategoryMappingMarkdownReport(data, missing, lowCoverage) {
...
`;
    fs_1.default.writeFileSync(path_1.default.join(REPORTS_DIR, 'category-mapping-report.md'), md);
}
function generateCleaningMarkdownReport(data) {
...
`;
    fs_1.default.writeFileSync(path_1.default.join(REPORTS_DIR, 'cleaning-report.md'), md);
}
function explainProduct(catalog, id) {
    ...
    const classResult = (0, relevanceClassifier_js_1.classifyProduct)(name, rawCat, desc, specs);
    console.log(`\n======================================================`);
    ...
    const extractedDims = (0, dimensions_js_1.extractDimensions)(specs, name, desc, classResult.canonicalCategory);
    const sanitizedDims = (0, dimensionsSanitizer_js_1.sanitizeDimensions)({
    ...
    }, name, desc, classResult.canonicalCategory);
