...
const weightsPath = path_1.default.join(process.cwd(), 'config/quality-weights.json');
const QUALITY_CONFIG = JSON.parse(fs_1.default.readFileSync(weightsPath, 'utf8'));
/**
 * Calculates a quality score from 0-100 for normalized products.
 */
function calculateQualityScore(product, category) {
    const issues = [];
    let score = 0;
    ...
    const hasPrimaryImage = images.some((img) => img.isPrimary && img.url && img.url.startsWith('http'));
    if (hasPrimaryImage) {
    ...
    if (mats.length > 0 && !mats.includes('Wood') && mats.some((m) => m !== 'Wood')) {
        score += w.materials;
    }
