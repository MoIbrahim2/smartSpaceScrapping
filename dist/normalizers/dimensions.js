 ...
 */
function extractDimensions(specs = {}, title = '', description = '', category = '') {
    let width = null;
    let height = null;
    let depth = null;
    let weight = null;
    ...
    if (amzWidthMatch)
        width = normalizeToCm(parseFloat(amzWidthMatch[1]), 'cm');
    const amzHeightMatch = combinedText.match(/(\d+(?:\.\d+)?)\s*(?:الارتفاع|الإرتفاع)/i);
    if (amzHeightMatch)
        height = normalizeToCm(parseFloat(amzHeightMatch[1]), 'cm');
    const amzDepthMatch = combinedText.match(/(\d+(?:\.\d+)?)\s*(?:العمق|الطول|طول)/i);
    if (amzDepthMatch)
        depth = normalizeToCm(parseFloat(amzDepthMatch[1]), 'cm');
    ...
    if (width === null) {
        ...
        if (nWidth)
            width = normalizeToCm(parseFloat(nWidth[1]), 'cm');
    }
    if (height === null) {
        const nHeight = combinedText.match(/Height\s*\((?:cm|m|سم)\)\s*(\d+(?:\.\d+)?)/i);
        if (nHeight)
            height = normalizeToCm(parseFloat(nHeight[1]), 'cm');
    }
    if (depth === null) {
        const nDepth = combinedText.match(/Depth\s*\((?:cm|m|سم)\)\s*(\d+(?:\.\d+)?)/i);
        if (nDepth)
            depth = normalizeToCm(parseFloat(nDepth[1]), 'cm');
    }
    ...
    for (const [key, value] of Object.entries(specs)) {
        ...
        if (kLower.includes('مقعد') || kLower.includes('seat'))
            continue;
    ...
    if (width === null || height === null || depth === null) {
        const match3D = combinedText.match(/(\d+(?:\.\d+)?)\s*(?:m|cm|mm|م|سم)?\s*[*x×]\s*(\d+(?:\.\d+)?)\s*(?:m|cm|mm|م|سم)?\s*[*x×]\s*(\d+(?:\.\d+)?)\s*(?:m|cm|mm|م|سم)?/i);
        if (match3D) {
    ...
    if (width === null)
        width = catDefault.width;
    if (depth === null)
        depth = catDefault.depth;
    if (height === null)
        height = catDefault.height;
    if (weight === null)
...
function normalizeToCm(value, context) {
