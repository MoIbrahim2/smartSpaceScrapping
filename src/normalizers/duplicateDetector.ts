 ...
 */
export function detectAndProcessDuplicates(products: UnifiedProduct[]): {
  ...
  const exactMap = new Map<string, UnifiedProduct>();
  ...
  const duplicateCandidatesReport: DuplicateGroup[] = [];
  ...
  for (let i = 0; i < mergedProducts.length; i++) {
    ...
    const candidates: DuplicateGroup['candidateProductIds'] = [];
    ...
    for (let j = 0; j < mergedProducts.length; j++) {
      ...
      if (p1.source.marketplace === p2.source.marketplace) continue;
      ...
      if (
        ...
        if (
          d1.width !== null && d2.width !== null && Math.abs(d1.width - d2.width) <= 5 &&
