...
import { enrichProductAI } from './aiEnrichment.js';
...
const SEED_DIMENSIONS: Record<string, { width: number[]; depth: number[]; height: number[] }> = {
...
const CATEGORY_ROOM_MAP: Record<string, string[]> = {
 ...
 */
export function expandCatalog(existingCleanProducts: UnifiedProduct[], targetPerCategory = 25): UnifiedProduct[] {
  ...
  const categoryGroups: Record<string, UnifiedProduct[]> = {};
  ...
  const expandedList: UnifiedProduct[] = [...existingCleanProducts];
  ...
  ALL_CANONICAL_CATEGORIES.forEach(category => {
    ...
    let imagePool: { url: string; isPrimary: boolean }[][] = currentItems
    ...
    for (let i = 0; i < needed; i++) {
      ...
      let width: number, depth: number, height: number, price: number, name: string, desc: string;
      ...
      const qRes = calculateQualityScore(newProduct, category);
